      ...
      tasks: [
        {
          taskName: 'Clean',
          operators: [
            { name: 'KNN Padding', value: 'clean-knn', active: true },
            { name: 'Mean Padding', value: 'clean-mean', active: true },
            { name: 'Elimination', value: 'elimination', active: false }
          ],
          taskValue: 'clean',
          required: true
        },
        {
          taskName: 'Normalize',
          operators: [
            { name: 'MinMax Scaler', value: 'normalize', active: true },
            { name: 'Z-score Scaler', value: 'km-padding', active: false },
            { name: 'Decimal Scaler', value: 'elimination', active: false }
          ],
          taskValue: 'normalize',
          bgColor: '94bfa7'
        },
        {
          taskName: 'Transform',
          operators: [
            { name: 'Linear Transformation', value: 'transform', active: true },
            { name: 'Cuadratic Transformation', value: 'km-padding' }
          ],
          taskValue: 'transform',
          bgColor: 'e0b7b7'
        },
        {
          ...
          operators: [
            {
              name: 'Principal Component Analisis (PCA)',
              value: 'reduce',
              active: true
            },
            { name: 'IsoMap', value: 'km-padding' },
            {
              name: 'Composite Least Square Projection (CLSP)',
              value: 'km-padding'
            }
          ],
          taskValue: 'reduce',
        ...
        },
        {
          ...
          operators: [
            {
              name: 'Cyclicity operator',
              value: 'cyclicity',
              active: true
            },
            {
              name: 'Spiral Visualization',
              value: 'spiral',
              active: true
            }
          ],
      ...
      <table class="taskTable">
        <tr>
            ${actions
              .map(
                (action, idx) =>
                  `<td id="op-${action.value}" class="border operator ${
                    action.active ? 'active' : ''
                  }"
                  data-value="${action.value}" data-taskvalue="${
                    task.taskValue
                  }" data-opname="${action.name}"
                  title="${action.name}">
                      ${idx + 1}
                  </td>`
              )
          ...
          <tr >
            <td id="task-${task.taskValue}" colspan="${
        actions.length
      }" style="vertical-align:middle; background-color: #${bgColor};" class="border task show-info">${taskName}</td>
        </tr>
      </table>`
      return template
    }
