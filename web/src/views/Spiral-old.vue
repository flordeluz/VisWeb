...
export default {
	...
	renderSpiral() {
	    ...
	    // var start_year = 2009
	    let sampled_data = this.sampleArrayByYear(this.raw_data, 12 / this.params.points_per_period)
		...
		size = raw_data[index][this.feature].map(0, this.max_values[feat_idx], 1, 10)
		// THIS LINE IS OVERRIDING THE MAP IN THE PREVIOUS ONE
	...
	async getPeriodRecommendations() {
		...
		`http://localhost:8081/data/${this.station}/period?feature=${feat_idx}`,
		{ crossdomain: true }
	    )
	...
	async showDatasetSpiral() {
		...
		// "http://localhost:8080/data/" + this.dataset + "/" + this.station + "/D",
		"http://localhost:8081/data/" + this.station + "/scale",
		{ crossdomain: true }
	    )
